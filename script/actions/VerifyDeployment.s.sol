...
contract VerifyDeployment is Config {
    ...
    function run() public {
        ...
        initialGovernor = vm.parseJsonAddress(
            vm.readFile(getAuthAddressesPath(protocolId)), ".initialGovernor"
        );
        ...
        logBoolCheck(
            "Factory Initialized",
            readInitialized(p.factoryAddress) > 0
                && Factory(a.factory).proxiableUUID() == implSlot
        );
        logAddressCheck(
        ...
        logBoolCheck(
            "Oracle Engine Initialized", readInitialized(p.oracleEngineAddress) > 0
        );
        logAddressCheck(
            "Oracle Engine Beacon",
            a.oracleEngineBeacon,
            readProxyBeacon(p.oracleEngineAddress)
        );
        ...
        logBoolCheck(
            "Chainlink-C Initialized",
            readInitialized(p.chainlinkCompositeProxy) > 0
                && Factory(a.chainlinkOracleComposite).proxiableUUID() == implSlot
        );
        ...
        logBoolCheck(
            "Chainlink Initialized",
            readInitialized(p.chainlinkProviderProxy) > 0
                && Factory(a.chainlinkOracleProvider).proxiableUUID() == implSlot
        );
        ...
        logBoolCheck(
            "Pyth Initialized",
            readInitialized(p.pythProviderProxy) > 0
                && Factory(a.pythOracleProvider).proxiableUUID() == implSlot
        );
        ...
        logBoolCheck(
            "Timelock has CONFIGURATOR role",
            IRBAC(p.riskEngineAddress).hasPermission(
                bytes32("CONFIGURATOR"), p.timelockAddress
            )
        );
        logBoolCheck(
            "Timelock has BORROW GUARDIAN role",
            IRBAC(p.riskEngineAddress).hasPermission(
                bytes32("BORROW_CAP_GUARDIAN"), p.timelockAddress
            )
        );
        logBoolCheck(
            "Timelock has SUPPLY GUARDIAN role",
            IRBAC(p.riskEngineAddress).hasPermission(
                bytes32("SUPPLY_CAP_GUARDIAN"), p.timelockAddress
            )
        );
        logBoolCheck(
            "Timelock has RESERVE_MANAGER role",
            IRBAC(p.riskEngineAddress).hasPermission(
                bytes32("RESERVE_MANAGER"), p.timelockAddress
            )
        );
        logBoolCheck(
            "Timelock has RESERVE_WITHDRAWER role",
            IRBAC(p.riskEngineAddress).hasPermission(
                bytes32("RESERVE_WITHDRAWER"), p.timelockAddress
            )
        );
    ...
    function logBoolCheck(string memory label, bool actual) internal pure {
